...
use super::{Arity, Category, InputDescriptor, TickCtx, UGen, Unit, signal};

pub(super) static UGENS: &[UGen] = &[
    // pan2 ( in pos -- l r )  state: [key, gl, gr]   pos -1 = hard left .. +1 = hard right
    UGen {
        name: "pan2",
        category: Category::Panner,
        description: "Equal-power stereo panner — mono in, L/R out; `pos` -1 (left) … +1 (right).",
        examples: &[
            "440 sine 0.2 * 0 pan2 out",
            "440 sine 0.2 *  0.5 sine  pan2 out",
        ],
        arity: Arity::Fixed(2),
        inputs: &[
            signal("in"),
