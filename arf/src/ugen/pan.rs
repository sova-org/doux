...
pub(super) static UGENS: &[UGen] = &[
    ...
    UGen { name: "pan2", category: Category::Panner, description: "Equal-power stereo panner — mono in, L/R out; `pos` -1 (left) … +1 (right).",
           ...
           inputs: &[signal("in"), InputDescriptor { name: "pos", unit: Unit::Ratio, range: (-1.0, 1.0), default: 0.0, rate: Rate::Audio }],
           outputs: 2, state_slots: 0, buffer_len: 0, rate: Rate::Audio, cost: 10, tick: tick_pan2 },
];

// `.max(-1).min(1)` not `.clamp()`: it suppresses NaN the way the JIT's min/max shims do, so
// both backends stay bit-exact on a NaN `pos` (the SVF core clamps the same way, and why).
#[allow(clippy::manual_clamp)]
fn tick_pan2(ctx: &mut TickCtx, out: &mut [f32]) {
    let x = ctx.inputs[0];
    // Clamp pos via max/min (not `.clamp()`) to mirror the JIT's NaN-suppressing shims, then
