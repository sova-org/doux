...
impl LadderDsp {
	...
	pub fn compute(
		...
		let mut fSlow9: F32 = 4.0 * fSlow7 * (1.0 - fSlow5);
		let zipped_iterators = inputs0.zip(outputs0);
		for (input0, output0) in zipped_iterators {
			let mut fTemp0: F32 = *input0;
			let mut fTemp1: F32 = fSlow8 * (fTemp0 - fSlow9 * (self.fRec3[1] + fSlow5 * (self.fRec2[1] + fSlow5 * (self.fRec1[1] + fSlow5 * self.fRec0[1])))) - self.fRec0[1];
