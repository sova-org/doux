...
impl FeedbackDsp {
	...
	pub fn compute(
		...
		for ((((input0, input1), input2), output0), output1) in zipped_iterators {
			...
			let mut fTemp9: F32 = fTemp3 * self.fRec6[1] + fTemp4 * self.fRec1[1];
			self.fRec6[0] = (if (F32::abs(fTemp9) > 1.1754944e-38) as i32 != 0 {fTemp9} else {0.0});
			let mut fTemp10: F32 = *input1 + fTemp1 * (self.fRec3[0] * fTemp8 + fTemp7 * self.fRec6[0]);
			self.fVec0[(self.IOTA0 & 131071) as usize] = fTemp10;
			let mut fTemp11: F32 = F32::max(1.0, F32::min(65535.0, self.fConst0 / *input0));
			let mut iTemp12: i32 = (fTemp11) as i32;
			let mut iTemp13: i32 = std::cmp::min(65537, std::cmp::max(0, iTemp12));
			let mut fTemp14: F32 = F32::floor(fTemp11);
			let mut fTemp15: F32 = fTemp14 + (1.0 - fTemp11);
			let mut fTemp16: F32 = fTemp11 - fTemp14;
			let mut iTemp17: i32 = std::cmp::min(65537, std::cmp::max(0, i32::wrapping_add(iTemp12, 1)));
			let mut fTemp18: F32 = self.fVec0[((i32::wrapping_sub(self.IOTA0, iTemp13)) & 131071) as usize] * fTemp15 + fTemp16 * self.fVec0[((i32::wrapping_sub(self.IOTA0, iTemp17)) & 131071) as usize];
			self.fRec0[0] = (if (F32::abs(fTemp18) > 1.1754944e-38) as i32 != 0 {fTemp18} else {0.0});
			let mut fTemp19: F32 = *input2 + fTemp1 * (self.fRec3[0] * fTemp7 + fTemp8 * self.fRec6[0]);
			self.fVec1[(self.IOTA0 & 131071) as usize] = fTemp19;
			let mut fTemp20: F32 = fTemp15 * self.fVec1[((i32::wrapping_sub(self.IOTA0, iTemp13)) & 131071) as usize] + fTemp16 * self.fVec1[((i32::wrapping_sub(self.IOTA0, iTemp17)) & 131071) as usize];
			self.fRec1[0] = (if (F32::abs(fTemp20) > 1.1754944e-38) as i32 != 0 {fTemp20} else {0.0});
