...
Compilation options: -lang rust -ct 1 -cn FeedbackDsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0
------------------------------------------------------------ */
#[repr(C)]
...
pub const FAUST_INPUTS: usize = 3;
...
impl FeedbackDsp {
	...
	pub fn metadata(&self, m: &mut dyn Meta) { 
		...
		m.declare("compile_options", r"-lang rust -ct 1 -cn FeedbackDsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0");
	...
	pub fn instance_constants(&mut self, sample_rate: i32) {
		...
		self.fConst3 = 0.001 * self.fConst0;
	}
	...
	pub fn init(&mut self, sample_rate: i32) {
		FeedbackDsp::class_init(sample_rate);
	...
	pub fn build_user_interface_static(ui_interface: &mut dyn UI<FaustFloat>) {
		ui_interface.open_vertical_box("feedback");
		ui_interface.add_horizontal_slider("b_damp", ParamIndex(0), 0.0, 0.0, 1.0, 0.001);
		ui_interface.add_horizontal_slider("c_cross", ParamIndex(1), 0.0, 0.0, 1.0, 0.001);
		ui_interface.add_horizontal_slider("g_fb", ParamIndex(2), 0.0, 0.0, 0.99, 0.001);
	...
	pub fn compute(
		...
		// Obtaining locks on 0 static var(s)
		let [inputs0, inputs1, inputs2, .. ] = inputs.as_ref() else { panic!("wrong number of input buffers"); };
		...
		for ((((input0, input1), input2), output0), output1) in zipped_iterators {
			...
			let mut fTemp0: F32 = F32::max(0.0, F32::min(0.99, self.fRec2[0]));
			self.fRec4[0] = fSlow1 + self.fConst1 * self.fRec4[1];
			let mut fTemp1: F32 = 1.0 - self.fRec4[0];
			self.fRec3[0] = self.fRec4[0] * self.fRec3[1] + fTemp1 * self.fRec0[1];
			self.fRec5[0] = fSlow2 + self.fConst1 * self.fRec5[1];
			let mut fTemp2: F32 = F32::max(0.0, F32::min(1.0, self.fRec5[0]));
			let mut fTemp3: F32 = 1.0 - fTemp2;
			self.fRec6[0] = self.fRec4[0] * self.fRec6[1] + fTemp1 * self.fRec1[1];
			let mut fTemp4: F32 = *input1 + fTemp0 * (self.fRec3[0] * fTemp3 + fTemp2 * self.fRec6[0]);
			self.fVec0[(self.IOTA0 & 255) as usize] = fTemp4;
			let mut fTemp5: F32 = F32::max(1.0, F32::min(131071.0, self.fConst3 * *input0));
			let mut iTemp6: i32 = (fTemp5) as i32;
			let mut iTemp7: i32 = std::cmp::min(131073, std::cmp::max(0, iTemp6));
			let mut fTemp8: F32 = F32::floor(fTemp5);
			let mut fTemp9: F32 = fTemp8 + (1.0 - fTemp5);
			let mut fTemp10: F32 = fTemp5 - fTemp8;
			let mut iTemp11: i32 = std::cmp::min(131073, std::cmp::max(0, i32::wrapping_add(iTemp6, 1)));
			self.fRec0[0] = self.fVec0[((i32::wrapping_sub(self.IOTA0, iTemp7)) & 255) as usize] * fTemp9 + fTemp10 * self.fVec0[((i32::wrapping_sub(self.IOTA0, iTemp11)) & 255) as usize];
			let mut fTemp12: F32 = *input2 + fTemp0 * (self.fRec3[0] * fTemp2 + fTemp3 * self.fRec6[0]);
			self.fVec1[(self.IOTA0 & 255) as usize] = fTemp12;
			self.fRec1[0] = fTemp9 * self.fVec1[((i32::wrapping_sub(self.IOTA0, iTemp7)) & 255) as usize] + fTemp10 * self.fVec1[((i32::wrapping_sub(self.IOTA0, iTemp11)) & 255) as usize];
