...
Compilation options: -lang rust -ec -ct 1 -cn CombDsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 1
------------------------------------------------------------ */
#[repr(C)]
pub struct CombDsp {
	fSampleRate: i32,
...
impl CombDsp {
	...
	pub fn metadata(&self, m: &mut dyn Meta) { 
		...
		m.declare("compile_options", r"-lang rust -ec -ct 1 -cn CombDsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 1");
	...
	pub fn compute(
		...
		for ((input0, input1), output0) in zipped_iterators {
			...
			let mut fTemp2: F32 = fTemp1 * self.fRec1[1] + (1.0 - fTemp1) * self.fRec0[1];
			self.fRec1[0] = (if (F32::abs(fTemp2) > 1.1754944e-38) as i32 != 0 {fTemp2} else {0.0});
			let mut fTemp3: F32 = self.fSlow1 + self.fConst1 * self.fRec3[1];
			self.fRec3[0] = (if (F32::abs(fTemp3) > 1.1754944e-38) as i32 != 0 {fTemp3} else {0.0});
			let mut fTemp4: F32 = *input1 + self.fRec1[0] * F32::max(-0.99, F32::min(0.99, self.fRec3[0]));
			self.fVec0[(self.IOTA0 & 16383) as usize] = fTemp4;
			let mut fTemp5: F32 = F32::max(1.0, F32::min(8191.0, self.fConst0 / *input0));
			let mut iTemp6: i32 = (fTemp5) as i32;
			let mut fTemp7: F32 = F32::floor(fTemp5);
			let mut fTemp8: F32 = self.fVec0[((i32::wrapping_sub(self.IOTA0, std::cmp::min(8193, std::cmp::max(0, iTemp6)))) & 16383) as usize] * (fTemp7 + (1.0 - fTemp5)) + (fTemp5 - fTemp7) * self.fVec0[((i32::wrapping_sub(self.IOTA0, std::cmp::min(8193, std::cmp::max(0, i32::wrapping_add(iTemp6, 1))))) & 16383) as usize];
			self.fRec0[0] = (if (F32::abs(fTemp8) > 1.1754944e-38) as i32 != 0 {fTemp8} else {0.0});
