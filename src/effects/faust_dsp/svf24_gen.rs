...
Compilation options: -lang rust -ct 1 -cn Svf24Dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0
------------------------------------------------------------ */
#[repr(C)]
pub struct Svf24Dsp {
	fHslider0: F32,
...
impl Svf24Dsp {
	...
	pub fn metadata(&self, m: &mut dyn Meta) { 
		m.declare("compile_options", r"-lang rust -ct 1 -cn Svf24Dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0");
	...
	pub fn compute(
		...
		let mut fSlow11: F32 = fSlow3 / fSlow4;
		let mut fSlow12: F32 = 1.0 / fSlow4;
		let zipped_iterators = inputs0.zip(outputs0);
		for (input0, output0) in zipped_iterators {
			let mut fTemp0: F32 = *input0;
			let mut fTemp1: F32 = self.fRec4[1] + fSlow3 * (fTemp0 - self.fRec5[1]);
			self.fRec4[0] = fSlow8 * fTemp1 - self.fRec4[1];
			let mut fTemp2: F32 = self.fRec5[1] + fSlow9 * fTemp1;
			self.fRec5[0] = 2.0 * fTemp2 - self.fRec5[1];
			let mut fRec6: F32 = fSlow10 * fTemp1;
			let mut fRec7: F32 = fTemp2;
			let mut fTemp3: F32 = (if iSlow1 != 0 {fRec7} else {(if iSlow2 != 0 {fTemp0 - (fRec7 + fSlow6 * fRec6)} else {fRec6})});
			let mut fTemp4: F32 = self.fRec0[1] + fSlow3 * (fTemp3 - self.fRec1[1]);
			self.fRec0[0] = fSlow5 * fTemp4 - self.fRec0[1];
			let mut fTemp5: F32 = self.fRec1[1] + fSlow11 * fTemp4;
			self.fRec1[0] = 2.0 * fTemp5 - self.fRec1[1];
			let mut fRec2: F32 = fSlow12 * fTemp4;
			let mut fRec3: F32 = fTemp5;
			*output0 = (if iSlow1 != 0 {fRec3} else {(if iSlow2 != 0 {fTemp3 - (fRec3 + 1.4144272 * fRec2)} else {fRec2})});
