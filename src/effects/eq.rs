...
impl Eq {
    ...
    pub fn process(&mut self, input: f32, lo_db: f32, mid_db: f32, hi_db: f32, sr: f32) -> f32 {
        let mut out = input;
        if lo_db != 0.0 {
            out = self.lo.process_with_gain(out, FilterType::Lowshelf, LO_FREQ, SHELF_Q, lo_db, sr);
        }
        if mid_db != 0.0 {
            out = self.mid.process_with_gain(out, FilterType::Peaking, MID_FREQ, MID_Q, mid_db, sr);
        }
        if hi_db != 0.0 {
            out = self.hi.process_with_gain(out, FilterType::Highshelf, HI_FREQ, SHELF_Q, hi_db, sr);
        }
        out
    }
}
