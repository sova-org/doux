...
impl LadderFilter {
    #[inline]
    fn refresh_coeffs(&mut self, cutoff: f32, sr: f32) {
    ...
    #[inline]
    fn tick(&mut self, input: f32, g: f32, res: f32, inv_2sr: f32, mode: LadderMode) -> f32 {
    ...
    pub fn process(
        ...
        self.refresh_coeffs(cutoff, sr);
        let res = resonance.clamp(0.0, 1.0) * 4.0;
        self.tick(input, self.cached_g, res, self.cached_inv_2sr, mode)
    }
    ...
    pub fn process_block(
        ...
        for slot in buf.iter_mut().take(n) {
            slot[ch] = self.tick(slot[ch], g, res, inv_2sr, mode);
        }
    }
}
